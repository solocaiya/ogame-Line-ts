          ...
          <SidebarMenu>
            <SidebarMenuItem v-for="item in navItems" :key="item.path">
              ...
              >
                ...
                <!-- 签到红点 -->
                <SidebarMenuBadge
                  v-if="item.path === '/checkin' && canCheckIn"
                  class="bg-destructive text-destructive-foreground"
                >
                  !
                </SidebarMenuBadge>
...
<script setup lang="ts">
  ...
  import { canCheckInToday } from '@/logic/checkInLogic'
  ...
  const navItems = computed(() => [
    ...
    { name: computed(() => t('nav.alliance')), path: '/alliance', icon: Flag },
  ...
  watch(() => !!authStore.accessToken, async (loggedIn) => {
    if (loggedIn) {
      ...
      connectWebSocket()
      // 初始化联盟和聊天系统
      allianceStore.init(gameStore.player.id)
      chatStore.init()
      ...
      // 登出，停止自动存档，断开 WebSocket，清理联盟和聊天
      stopCloudAutoSave()
      disconnectWebSocket()
      allianceStore.destroy()
      chatStore.destroy()
    }
  })
